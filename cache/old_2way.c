...
struct exmem lat3[2];
struct memwb lat4[2];
struct cache instca[2][256];
struct cache dataca[2][256];
...
void main(int argc, char *argv[]){
	...
	memset(instca,0,sizeof(struct cache)*2*256);
	memset(dataca,0,sizeof(struct cache)*2*256);
	memset(oldest_data,0,sizeof(int)*2*256);
	memset(oldest_inst,0,sizeof(int)*2*256);
...
int readmem(int addr, struct cache (* cacheline)[256], int num, int diff, int (*oldest)[256]){
	...
	if(cacheline[num][index].valid==0){ // cold miss : mem->cache 
		...
		if(cacheline[num][index].tag!=tag){
			if(num==1){ //select oldest
				int old1, old2, erase;
				old1=0; old2=0; erase=0;
				for(int x=0;x<2;x++){
					if(oldest[x][index]==1)
						old1=x;
