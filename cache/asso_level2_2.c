...
struct exmem lat3[2];
struct memwb lat4[2];
struct cache lv1_inst[2][256];
struct cache lv1_data[2][256];
struct cache lv2[4][256];
int oldest_inst[2][256];
int oldest_data[2][256];
int oldest_lv2[4][256];
...
void main(int argc, char *argv[]){
	...
	memset(lv1_inst,0,sizeof(struct cache)*2*256);
	memset(lv1_data,0,sizeof(struct cache)*2*256);
	memset(lv2,0,sizeof(struct cache)*4*256);
...
void fetch(){
	...
	lat1[0].inst=readmem(pc, lv1_inst, 0, L1Inst, oldest_inst);
...
int windex1, woffset1, wnum1, windex2, woffset2, wnum2, last;
...
int readmem(int addr, struct cache (* cacheline)[256], int num, int diff, int (* oldest)[256]){
	...
	if(cacheline[num][index].valid==0){ // cold miss : mem->cache 
		...
		if((diff==L1Inst)||(diff==L1Data)){ //Lv1
			int exist=4;
			int exist1, exist2, exist3;
			exist1=0;exist2=0;exist3=0;
		  	int	lv2num=0;
			while(1){
				for(int x=0; x<4; x++){
					if((lv2[x][index].valid==1)&&(lv2[x][index].tag==tag)){
						lv2num=x;
						exist=1;
						break;
					}
					else if((lv2[x][index].valid==1)&&(lv2[x][index].tag!=tag)){
						lv2num=x;
						exist2++;
					}
					else if(lv2[x][index].valid==0){
						exist3++;
					}
				}
				if(exist==1){ //L2 cacheline tags has L1's tag
					break;
				}
				else if(exist3){// L2 has empty cacheline
					readmem(addr, lv2, 0, L2, oldest_lv2); // l2_miss++ l2_hit++
					return readmem(addr, cacheline, num, diff, oldest);//l1_
				}
				else if(exist2==4){ //L2 is full with other tags
					readmem(addr, lv2, 0, L2, oldest_lv2);// l2_miss++ l2_miss++ l2+hit++
					return readmem(addr, cacheline, num, diff, oldest); //miss->hit = 0
				}
				else if(exist==4){
				}
			}
			for(int x=0;x<16;x++){
				cacheline[num][index].data[x]=lv2[lv2num][index].data[x];
			}
		}
		else if(diff==L2){ // Lv2
		...
		if(cacheline[num][index].tag!=tag){
			int comp=last-1;
			if(num==comp){ //select oldest
				int old1, old2, old3, old4, erase;
				old1=0; old2=0; old3=0; old4=0; erase=0; 
				for(int x=0; x<last; x++){
					if(oldest[x][index]==1)
						old1=x;
				...
				if(diff==2){
					...
					if(cacheline[num][index].dirty){
						int l2_num=6;
						int addr_for_data=((cacheline[num][index].tag<<14)|(index<<6));
						while(1){
							for(int x=0;x<4;x++){
								if((lv2[x][index].valid)&&(lv2[x][index].tag==cacheline[num][index].tag)){
									l2_num=x;
									break;
								}
							}
							if(l2_num==6){ // L1 exist, L2 not exist
								readmem(addr_for_data, lv2, 0, L2, oldest_lv2);
							}
							else{
								for(int x=0; x<16; x++){
									lv2[l2_num][index].data[x]=cacheline[num][index].data[x];
									lv2[l2_num][index].dirty=1;
								}
								break;
							}
						}
			...
			else if(cacheline[num][index].first==1){
...
void writemem(int addr, int value, struct cache (*cacheline)[256]){
	readmem(addr, cacheline, 0, L1Data, oldest_data);
	cacheline[wnum1][windex1].data[woffset1/4]=value;
	cacheline[wnum1][windex1].dirty=1;
/*
	for(int x=0;x<4;x++){
		if(cacheline[wnum1][windex1].tag==lv2[x][windex1].tag){
			lv2[x][windex1].dirty=1;
			break;
		}
	}	
*/	
