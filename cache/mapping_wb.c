...
int readmem(int addr, struct cache* cacheline, int diff);
void writemem(int addr, int value, struct cache* cacheline);

struct ifid lat1[2];
struct idex lat2[2];
struct exmem lat3[2];
struct memwb lat4[2];
struct cache instca[4];
struct cache dataca[4];
...
//	memset(cacheline,0,sizeof(struct cache)*256);
	memset(instca,0,sizeof(struct cache)*4);
	memset(dataca,0,sizeof(struct cache)*4);
...
void fetch(){
	...
	lat1[0].inst=readmem(pc, instca, 0);
	lat1[0].pc=pc;
	lat1[0].pc4=pc+4;
	lat1[0].valid=1;
...
int readmem(int addr, struct cache* cacheline, int diff){
	...
	tag=(addr&0xFFFFFF00)>>8;
	index=(addr&0x000000C0)>>6;
	offset=(addr&0x0000003F);

	windex, woffset=0;
	if(cacheline[index].valid==0){ // cold miss : mem->cache 

		int addr_for_data=addr&0xFFFFFFC0;
		for(int x=0;x<16;x++){
			cacheline[index].data[x]=mem[addr_for_data/4];
		...
		readmem(addr, cacheline, diff);
	}

	else if(cacheline[index].valid==1){ // cache exist

		if(cacheline[index].tag!=tag){

			if(cacheline[index].dirty){ // miss->erase
				int addr_for_data=(cacheline[index].tag<<8)|(index<<6);
				for(int x=0; x<16; x++){
					mem[addr_for_data/4]=cacheline[index].data[x];
					addr_for_data=addr_for_data+4;
				}
			}
			if(diff)
				...
				miss_inst++;
			cacheline[index].valid=0;
			readmem(addr, cacheline, diff);
		}
		
		else if(cacheline[index].tag==tag){ // hit
			if(diff)
				hit_data++;
			else if(diff==0)
				hit_inst++;
			...
			windex=index;
			woffset=offset;
			return cacheline[index].data[offset/4];			
		}
	}
}
...
void writemem(int addr, int value, struct cache *cacheline){
	...
	readmem(addr, cacheline, 1);
	cacheline[windex].data[woffset/4]=value;
	cacheline[windex].dirty=1;
	return ;
}
...
void memory(){
	...
	if(lat3[1].memRead){
		memtoResult=readmem(addr, dataca, 1);
		n_mem++;
