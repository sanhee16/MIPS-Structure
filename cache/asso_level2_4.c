...
struct exmem lat3[2];
struct memwb lat4[2];
struct cache lv1_inst[4][256];
struct cache lv1_data[4][256];
struct cache lv2[8][256];
int oldest_inst[4][256];
int oldest_data[4][256];
int oldest_lv2[8][256];
...
void main(int argc, char *argv[]){
	...
	memset(lv1_inst,0,sizeof(struct cache)*4*256);
	memset(lv1_data,0,sizeof(struct cache)*4*256);
	memset(lv2,0,sizeof(struct cache)*8*256);
	...
	if(cacheline[num][index].valid==0){ // cold miss : mem->cache 
		...
		if((diff==L1Inst)||(diff==L1Data)){ //Lv1
			...
			while(1){
				...
				if(exist==1){ //L2 cacheline tags has L1's tag
					...
					readmem(addr, lv2, 0, L2, oldest_lv2);
					return readmem(addr, cacheline, 0, diff, oldest);
				}
		...
		if(cacheline[num][index].tag!=tag){
			...
			if(num==comp){ //select oldest
				int old1, old2, old3, old4, old5, old6, old7, old8, erase;
				old1=0; old2=0; old3=0; old4=0; erase=0; 
				old5=0; old6=0; old7=0; old8=0;
				for(int x=0; x<last; x++){
					if(oldest[x][index]==1)
						old1=x;
						...
						old4=x;
					else if(oldest[x][index]==5)
						old5=x;
					else if(oldest[x][index]==6)
						old6=x;
					else if(oldest[x][index]==7)
						old7=x;
					else if(oldest[x][index]==8)
						old8=x;
				}
				int change=old1;
				while(1){
				...
				if(diff==2){
					...
					if(cacheline[num][index].dirty){
						...
						while(1){
							...
							if(l2_num==10){ // L1 exist, L2 not exist
